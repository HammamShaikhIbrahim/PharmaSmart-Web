...
if (!$user || !password_verify($data->old_pass, $user['Password'])) {
...
if (!empty($data->new_pass)) {
    ...
    $sql = "UPDATE User SET Email='$email', Password='$newHash' WHERE UserID=$user_id";
} else {
    $sql = "UPDATE User SET Email='$email' WHERE UserID=$user_id";
}
